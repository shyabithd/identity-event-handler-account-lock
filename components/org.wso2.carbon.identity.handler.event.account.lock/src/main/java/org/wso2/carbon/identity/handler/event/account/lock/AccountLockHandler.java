...
public class AccountLockHandler extends AbstractEventHandler implements IdentityConnectorConfig {
    ...
    private List<String> privilegeUserList;
    ...
    public void init(InitConfig initConfig) {
        ...
        privilegeUserList = new ArrayList<>();
        ClassLoader classLoader = getClass().getClassLoader();
        InputStream in = classLoader.getResourceAsStream("PrivilegeUsers");
        if (in != null) {
            BufferedReader rd = new BufferedReader(new InputStreamReader(in));
            String line;
            try {
                while ((line = rd.readLine()) != null) {
                    privilegeUserList.add(line);
                }
            } catch (IOException e) {
                log.error("Privilege user file does not exist");
            }
        }
    }
    ...
    protected boolean handlePostAuthentication(Event event, String userName, UserStoreManager userStoreManager,
        ...
        if ((Boolean) event.getEventProperties().get(IdentityEventConstants.EventProperty.OPERATION_STATUS)) {
            ...
            if (!privilegeUserList.contains(userName) && currentFailedAttempts >= maximumFailedAttempts) {
